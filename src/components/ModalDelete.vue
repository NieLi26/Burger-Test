<template>
    <TransitionRoot appear :show="isOpenDelete" as="template">
        <Dialog as="div" @close="setOpen(false)" class="relative z-50">
            ...
            <div class="fixed inset-0 overflow-y-auto">
                <div class="flex min-h-full items-center justify-center p-4 text-center">
                    <TransitionChild as="template" enter="duration-300 ease-out" enter-from="opacity-0 scale-95"
                        ...
                        <DialogPanel
                            class="flex flex-col max-w-md gap-2 p-6 rounded-md shadow-md bg-white">
                            ...
                            <p class="flex-1 dark:text-gray-400">Si eliminas el registro {{ props.hamburguesa.id }} no podras recuperarlo.</p>
                            <div class="flex flex-col justify-center gap-3 mt-6 sm:flex-row">
                                ...
                                <button @click="handleDeleteItem" class="px-6 py-2 rounded-sm shadow-sm bg-red-300 text-gray-900 dark:text-white">Confirmar</button>
                            </div>
                        </DialogPanel>
                    </TransitionChild>
                </div>
...
const handleDeleteItem = () => {
   store.deleteItem(props.hamburguesa.id)
   setOpen(false)
